...
def _analyze_pe_sections(pe: object) -> list[dict[str, str]]:
    """Extract section information from a PE binary."""
    sections = []
    for section in pe.sections:  # type: ignore[attr-defined]
        section_info = {
...
def _analyze_pe_data_directories(pe: object) -> list[dict[str, object]]:
    """Extract data directory information from a PE binary."""
    dir_names = [
        "Export Table",
        "Import Table",
        "Resource Table",
    ...
    data_directories: list[dict[str, object]] = []
    if hasattr(pe, "OPTIONAL_HEADER") and hasattr(pe.OPTIONAL_HEADER, "DATA_DIRECTORY"):  # type: ignore[attr-defined]
        for i, entry in enumerate(pe.OPTIONAL_HEADER.DATA_DIRECTORY):  # type: ignore[attr-defined]
            dir_info: dict[str, object] = {
                "index": i,
            ...
            data_directories.append(dir_info)

    return data_directories


def _annotate_data_directories(data_directories: list[dict[str, object]]) -> None:
    """Add notes to notable data directories."""
    # Check for Load Config Directory (entry 10)
    if len(data_directories) > 10:
        load_config = data_directories[10]
        if load_config["has_data"]:
            load_config["note"] = "Load Config present - may have absolute offsets"

    # Check for Debug Directory (entry 6)
    if len(data_directories) > 6:
        debug_dir = data_directories[6]
    ...
    if len(data_directories) > 9:
        tls_dir = data_directories[9]
        if tls_dir["has_data"]:
            tls_dir["note"] = "TLS Directory present - may have callback offsets"


def analyze_pe_binary(filepath: str | Path) -> dict[str, object]:
    ...
    sections = _analyze_pe_sections(pe)
    data_directories = _analyze_pe_data_directories(pe)
    _annotate_data_directories(data_directories)
...
def compare_analyses(working: dict[str, object], failing: dict[str, object]) -> dict[str, object]:
...
def main() -> None:
    ...
    with Path(output_path.replace(".json", ".txt")).open("w") as f:
        f.write(report_text)
