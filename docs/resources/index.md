# Resources

Additional resources for FlavorPack users and developers.

## 📚 Cookbook

Practical examples and recipes for common use cases.

### [Examples](../cookbook/examples/index.md)
Complete working examples for different application types:
- [CLI Tools](../cookbook/examples/cli-tool.md) - Command-line applications
- [Web Applications](../cookbook/examples/web-app.md) - Web servers and APIs

### [Recipes](../cookbook/recipes/index.md)
Step-by-step guides for specific tasks:
- [Docker Integration](../cookbook/recipes/docker.md) - Container workflows
- [CI/CD Pipelines](../cookbook/recipes/ci-cd.md) - Automated builds
- [Multi-Platform Builds](../cookbook/recipes/multi-platform.md) - Cross-platform packages

## 🔧 Troubleshooting

Solutions to common problems and error messages.

### Quick Links
- [Common Issues](../troubleshooting/common.md) - Frequently encountered problems
- [Error Messages](../troubleshooting/errors.md) - Error reference and solutions
- [FAQ](../troubleshooting/faq.md) - Frequently asked questions

### Platform-Specific Issues
- [macOS](../troubleshooting/platforms/macos.md) - macOS-specific troubleshooting
- [Linux](../troubleshooting/platforms/linux.md) - Linux distribution issues
- [Windows](../troubleshooting/platforms/windows.md) - Windows-specific problems

## 🛠️ Development

Resources for contributors and developers.

### Getting Involved
- [Contributing](../development/contributing.md) - How to contribute to FlavorPack
- [Architecture](../development/architecture.md) - System design and components
- [Building Helpers](../development/helpers.md) - Native component development

### Testing
- [Testing Guide](../development/testing/index.md) - Testing strategy
- [Unit Tests](../development/testing/unit.md) - Unit testing approach
- [Integration Tests](../development/testing/integration.md) - Integration testing
- [Cross-Language Tests](../development/testing/cross-language.md) - Multi-language testing

### Build & Release
- [CI/CD](../development/ci-cd.md) - Continuous integration setup
- [Release Process](../development/release.md) - Release workflow

## 👥 Community

Connect with the FlavorPack community.

- [Support](../community/support.md) - Get help and support
- [Discussions](../community/discussions.md) - Join the conversation
- [Blog](../community/blog.md) - News and updates

## Quick Navigation

### By Task

**I want to...**
- Build my first package → [First Package](../getting-started/first-package.md)
- Deploy to production → [Docker Integration](../cookbook/recipes/docker.md)
- Debug an issue → [Debugging Guide](../guide/advanced/debugging.md)
- Optimize performance → [Performance Guide](../guide/advanced/performance.md)
- Contribute code → [Contributing](../development/contributing.md)

### By Technology

**Working with...**
- Python → [Python Applications](../guide/packaging/python.md)
- Docker → [Docker Integration](../cookbook/recipes/docker.md)
- CI/CD → [CI/CD Pipelines](../cookbook/recipes/ci-cd.md)
- Multi-Platform → [Multi-Platform Builds](../cookbook/recipes/multi-platform.md)

### By Problem Type

**Troubleshooting...**
- Build failures → [Common Issues](../troubleshooting/common.md)
- Runtime errors → [Error Messages](../troubleshooting/errors.md)
- Platform issues → [Platform Troubleshooting](../troubleshooting/platforms/index.md)
- Performance problems → [Performance Guide](../guide/advanced/performance.md)

## Additional Resources

### External Links
- [GitHub Repository](https://github.com/provide-io/flavorpack)
- [Issue Tracker](https://github.com/provide-io/flavorpack/issues)
- [Discussions](https://github.com/provide-io/flavorpack/discussions)

!!! note "PyPI Package"
    FlavorPack is available via PyPI. See the [installation guide](../getting-started/installation.md).

### Documentation Formats
- [API Reference](../api/index.md) - Technical API documentation
- [User Guide](../guide/index.md) - Comprehensive user guide
- [PSPF Specification](../reference/spec/pspf-2025.md) - PSPF/2025 format specification
- [Glossary](../reference/glossary.md) - Definitions of key terms and concepts

## Getting Help

If you can't find what you're looking for:

1. Check the [FAQ](../troubleshooting/faq.md)
2. Search the [documentation](../index.md)
3. Browse [GitHub Issues](https://github.com/provide-io/flavorpack/issues)
4. Ask in [Discussions](https://github.com/provide-io/flavorpack/discussions)
5. Contact [Support](../community/support.md)
