...
1. [PSPF/2025 Format Specification](#pspf2025-format-specification)
1. [Multi-Language Components](#multi-language-components)
1. [Build Pipeline](#build-pipeline)
1. [Security Model](#security-model)
1. [Helper System](#helper-system)
...
1. **Progressive Extraction**: Load only what's needed, when needed
1. **Secure by Default**: Ed25519 signatures on all packages
1. **Zero Dependencies**: Launchers are static binaries
1. **Cross-Platform**: Works on Linux, macOS, Windows
1. **Reproducible**: Deterministic builds with seed keys
1. **Testable**: Comprehensive test suite with Taster
...
1. **To Flavor Pack**: A structured package with metadata and slots
...
1. **Dependencies Slot** - All pip packages as wheels
1. **Application Slot** - Application code
1. **Volatile Slots** - Temporary data removed after setup
...
1. **Local Development** - Built from source

   ```
   dist/bin/flavor-{go,rs}-{builder,launcher}-{platform}
   ```

1. **System Cache** - Downloaded or installed
...
1. **Integration Tests**: Multi-component interaction tests
1. **Cross-Language Tests**: Go/Rust/Python interoperability
1. **Security Tests**: Cryptographic verification
1. **Stress Tests**: Large files, concurrent operations
...
1. **Package Metadata** (`.{workenv}.pspf/package/`)
...
1. Verify extraction completion marker exists
1. Compare saved checksum with package checksum
1. Based on `FLAVOR_VALIDATION` level: warn/continue or fail
1. Strict validation: Fatal error on checksum mismatch
1. Re-extract if cache invalid or incomplete

## Key Design Decisions

1. **Multi-Language Architecture**: Combines strengths of Python (flexibility), Go/Rust (performance)
1. **Progressive Extraction**: Reduces startup time and disk usage
1. **Polyglot Format**: Single file works as both executable and package
1. **Built-in Security**: No external dependencies for verification
1. **Deterministic Builds**: Reproducible with seed keys
1. **Helper Independence**: Helpers are generic, data-driven executors

## Future Enhancements

1. **Windows Support**: Currently disabled due to UTF-8 issues
1. **Package Compression**: Additional compression algorithms
1. **Partial Updates**: Update slots without rebuilding entire package
1. **Network Loading**: Stream slots from remote sources
1. **Multi-Architecture**: Fat binaries with multiple architectures
