...
1. [Registry Structure](#2-registry-structure)
1. [Operation Categories](#3-operation-categories)
1. [Allocation Policies](#4-allocation-policies)
1. [Registration Procedures](#5-registration-procedures)
1. [Operation Specifications](#6-operation-specifications)
1. [Compatibility Requirements](#7-compatibility-requirements)
1. [Deprecation Process](#8-deprecation-process)
1. [Registry Governance](#9-registry-governance)
1. [Security Considerations](#10-security-considerations)
1. [IANA Considerations](#11-iana-considerations)
1. [Current Registry](#12-current-registry)
...
1. MUST have two independent implementations
1. MUST pass interoperability testing
1. MUST undergo security review for cryptographic operations
1. MUST be approved by Registry Board
...
1. MUST include test vectors
1. MUST demonstrate non-duplication
1. SHOULD have reference implementation
1. Expert reviewer approval required
...
1. MUST provide use cases
1. MUST include implementation
1. Expert panel evaluation required
...
1. MUST NOT appear in standard specifications
1. Implementations MAY refuse these operations
1. No interoperability guarantee
...
1. Expedited expert review (48 hours)
1. Emergency board approval
1. Immediate publication
1. Post-hoc public review
...
Frame := Magic Number + Frame Header + Data Blocks + [Content Checksum] Magic := 0xFD2FB528 (little-endian)
...
1. Deprecated operations MUST remain documented
1. Implementations SHOULD warn on deprecated operation use
1. Major versions MAY remove support for deprecated operations

### 7.3 Forward Compatibility

1. Unknown operations MUST cause explicit errors
1. Implementations MUST NOT skip unknown operations
1. Version negotiation SHOULD occur before processing
1. Clear error messages MUST indicate unsupported operations
...
**Terms**: 2 years, renewable once **Meetings**: Quarterly or as needed **Decisions**: Simple majority, chair breaks ties
...
1. Approves/rejects allocations
1. Manages deprecation process
1. Resolves disputes
1. Publishes annual registry report
...
1. PSPF Technical Committee
1. IETF/IANA (for process violations)
...
1. 90-day disclosure timeline
1. CVE assignment if applicable
1. Coordinated patch release
1. Registry update with warnings
