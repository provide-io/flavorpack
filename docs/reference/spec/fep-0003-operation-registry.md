...
This document defines the operation registry structure and governance for the full 256-code space. **Currently, only the v0 required operations (Section 12.1) are implemented** in the current v0 implementation. The remaining operations, categories, and procedures described here are tracked in the roadmap and serve as the design specification for the operation system.
