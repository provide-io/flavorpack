...
1. **Slot Table** (slot_count × 64 bytes)
...
1. **Magic Trailer** (8 bytes around index)
...
1. Seek to `EOF - 8200` and verify start magic (`📦`)
1. Read index block from `EOF - 8196` (8192 bytes)
1. Verify index checksum (Adler-32)
1. Verify package signature (Ed25519)
1. Read metadata from `metadata_offset` (size: `metadata_size`)
1. Read slot table from `slot_table_offset` (size: `slot_count × 64`)
1. Extract slots as needed based on lifecycle rules
