...
The Flavor Pack CI/CD system consists of multiple independent GitHub Actions workflows that build, test, and validate the entire system across multiple platforms and language implementations.
...
### 🔨 Ingredient Pipeline (`ingredient-pipeline.yml`)
...
**Scripts Used**:
- `.github/scripts/build-platform-ingredients.sh` - Build ingredients for specific platform
- `.github/scripts/test-metadata.py` - Generate build metadata
- `.github/scripts/run-tests.sh` - Run validation tests

### 🍷 Taster Pipeline (`taster-pipeline.yml`)

**Purpose**: Test the Taster comprehensive test package

**Trigger**: Manual or after ingredient pipeline completion
...
**Dependencies**:
- Requires successful ingredient pipeline run
- Downloads artifacts using `dawidd6/action-download-artifact`

### 🧪 Pretaster Pipeline (`pretaster-pipeline.yml`)
...
- uses: dawidd6/action-download-artifact@v6
  with:
    workflow: ingredient-pipeline.yml
...
### Unit Tests
- Run via pytest in ingredient pipeline
