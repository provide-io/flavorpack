...
func execBundleReplace(exePath string, args []string, userCwd string, logger hclog.Logger) error {
	...
	err = syscallExecFn(binary, argv, envv) // #nosec G204 -- the package launcher intentionally replaces the current process with the resolved target binary.
