...
func (sp *SlotProcessor) processSlot(index int, slot *Slot) error {
	slotIndex, err := intToUint64Checked(index, "slot index")
	if err != nil {
		return err
	}
	...
	compressedSize, err := intToUint64Checked(len(compressed), "compressed slot size")
	if err != nil {
		return err
	}
	originalSize, err := intToUint64Checked(len(slotData), "slot size")
	if err != nil {
		return err
	}
...
func (sp *SlotProcessor) loadSlotData(slot *Slot) ([]byte, []byte, uint8, error) {
	...
	// Read slot data
	slotData, err := readFileValidated(slotPath)
	if err != nil {
