...
#[cfg(test)]
mod tests {
    use super::SlotProcessor;
    use crate::psp::format_2025::builder::slot_processor::SELF_REF_MARKER;
    use crate::psp::format_2025::constants::{OP_GZIP, OP_TAR};
    use crate::psp::format_2025::defaults::DEFAULT_FILE_PERMS;
    use crate::psp::format_2025::manifest::ManifestSlot;
    use crate::psp::format_2025::operations::pack_operations;
    use tempfile::tempdir;

    fn manifest_slot(source: String) -> ManifestSlot {
        ManifestSlot {
            slot: None,
            id: "payload".to_string(),
            source,
            target: "app/payload.txt".to_string(),
    ...
    #[test]
    fn test_process_slots_self_ref_sets_defaults() {
        let mut processor = SlotProcessor::new(vec![ManifestSlot {
            source: SELF_REF_MARKER.to_string(),
            target: "bin/launcher".to_string(),
            purpose: "runtime".to_string(),
            lifecycle: "startup".to_string(),
            ..manifest_slot(SELF_REF_MARKER.to_string())
        }]);

        processor
            .process_slots()
            .expect("self-ref slot should process");

        assert_eq!(processor.metadata_slots.len(), 1);
        assert_eq!(processor.slot_descriptors.len(), 1);
        assert_eq!(processor.slot_paths.len(), 1);

        let metadata = &processor.metadata_slots[0];
        assert_eq!(metadata.permissions.as_deref(), Some("0600"));
        assert_eq!(metadata.resolution.as_deref(), Some("build"));
        assert_eq!(metadata.self_ref, Some(true));

        let descriptor = &processor.slot_descriptors[0];
        ...
        assert!(processor.slot_paths[0].as_os_str().is_empty());
    }

    #[test]
    fn test_process_slots_regular_file_builds_descriptor() {
        let tmp = tempdir().expect("tempdir should succeed");
        let slot_path = tmp.path().join("payload.txt");
        std::fs::write(&slot_path, b"payload-data").expect("write should succeed");

        let mut slot = manifest_slot(slot_path.display().to_string());
        slot.operations = "tgz".to_string();
        slot.permissions = Some("0755".to_string());
        slot.purpose = "tool".to_string();
        slot.lifecycle = "cache".to_string();
        slot.resolution = Some("runtime".to_string());

        let mut processor = SlotProcessor::new(vec![slot]);
        processor
            .process_slots()
            .expect("regular slot should process");

        let metadata = &processor.metadata_slots[0];
        assert_eq!(metadata.size, 12);
        assert!(metadata.checksum.starts_with("sha256:"));
        assert_eq!(metadata.permissions.as_deref(), Some("0755"));
        assert_eq!(metadata.resolution.as_deref(), Some("runtime"));
        ...
        assert_eq!(operations, pack_operations(&[OP_TAR, OP_GZIP]));
        assert_eq!(
            u16::from(descriptor.permissions_high) << 8 | u16::from(descriptor.permissions),
            0o755
        );
        assert_eq!(processor.slot_paths[0], slot_path);
    }

    #[test]
    fn test_process_slots_rejects_slot_number_mismatch() {
        let mut slot = manifest_slot("ignored".to_string());
        slot.slot = Some(1);

        let mut processor = SlotProcessor::new(vec![slot]);
        let err = processor
            .process_slots()
            .expect_err("slot number mismatch should fail");
        assert!(err.to_string().contains("Slot number mismatch"));
    }

    #[test]
    fn test_resolve_slot_path_uses_current_dir_when_workenv_base_is_unset() {
        let tmp = tempdir().expect("tempdir should succeed");
        std::fs::create_dir_all(tmp.path().join("nested")).expect("nested dir should exist");
        std::fs::write(tmp.path().join("nested/file.txt"), b"x").expect("test file should exist");
        ...
        let processor = SlotProcessor::new(Vec::new());

        let resolved = processor
            .resolve_slot_path("{workenv}/nested/file.txt")
        ...
        std::env::set_current_dir(original_dir).expect("set_current_dir restore should succeed");
    }

    #[test]
    fn test_create_slot_descriptor_uses_default_permissions_for_invalid_octal() {
        let processor = SlotProcessor::new(Vec::new());
        let mut slot = manifest_slot("ignored".to_string());
        slot.operations = "tar, gzip".to_string();
        slot.permissions = Some("invalid".to_string());

        let descriptor = processor
            .create_slot_descriptor(0, &slot, 5, 0xdead_beef)
