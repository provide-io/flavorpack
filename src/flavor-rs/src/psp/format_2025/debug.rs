...
mod tests {
    ...
    #[test]
    fn format_index_json_renders_expected_fields() {
        let mut index = Index::new();
        index.format_version = 0x2025_0001;
        index.package_size = 4096;
        index.launcher_size = 128;
        index.slot_count = 2;
        index.metadata_offset = 0xfeed;
        index.metadata_size = 512;
        index.public_key = [0xab; 32];

        let json = format_index_json(&index);

        assert!(json.contains(r#""version": "0x20250001""#));
        assert!(json.contains(r#""file_size": 4096"#));
        assert!(json.contains(r#""launcher_size": 128"#));
        assert!(json.contains(r#""descriptor_count": 2"#));
        assert!(json.contains(r#""metadata_offset": "0xfeed""#));
        assert!(json.contains(r#""metadata_size": 512"#));
        assert!(json.contains(r#""metadata_format": "JSON""#));
    ...
    #[test]
    fn analyze_helpers_accept_common_magic_values() {
        let result = catch_unwind(AssertUnwindSafe(|| {
            analyze_metadata_format(b"\x1f\x8b\x08\x00");
            analyze_metadata_format(br#"{\"format\":\"PSPF/2025\"}"#);
            analyze_metadata_format(b"ustar\0\0\0");
            analyze_metadata_format(b"plain");

            identify_slot_content(0, b"\x1f\x8b\x08\x00");
            identify_slot_content(1, br#"{\"slot\":1}"#);
            identify_slot_content(2, b"ustar\0\0\0");
            identify_slot_content(3, b"PK\x03\x04");
            identify_slot_content(4, b"plain");
        }));
    ...
    #[test]
    fn debug_dump_returns_error_for_invalid_bundle() {
        let temp = tempdir().expect("tempdir");
        let package = temp.path().join("bundle.psp");
        std::fs::write(&package, vec![0u8; 9000]).expect("write invalid bundle");
        let mut reader = Reader::new(&package).expect("create reader");
        let output = temp.path().join("debug");

        assert!(debug_dump(&mut reader, &output).is_err());
    }
}
