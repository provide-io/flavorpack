...
def extract_command(package_file: str, slot_index: int, output_path: str, force: bool) -> None:
    ...
    try:
        with PSPFReader(package_path) as reader:
            ...
            slot_name = slots_metadata[slot_index].get("name", f"slot_{slot_index}") if slot_index < len(slots_metadata) else f"slot_{slot_index}"
            click.echo(f"Extracting slot {slot_index}: {slot_name} ({format_size(slot.size)})")
            ...
            click.secho(f"✅ Extracted {format_size(len(data))} to {output}", fg="green")
...
def extract_all_command(package_file: str, output_dir: str, force: bool) -> None:
    ...
    try:
        with PSPFReader(package_path) as reader:
            ...
            click.echo(f"Extracting {len(slot_descriptors)} slots from {package_path.name}")
            ...
            for i, slot in enumerate(slot_descriptors):
                ...
                # Extract slot data  
                click.echo(f"📦 Extracting slot {i}: {slot_name} ({format_size(slot.size)})")
