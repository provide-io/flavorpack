...
def resolve_keys(config: KeyConfig) -> Tuple[bytes, bytes]:
    ...
    if config.has_seed():
        logger.info(f"🌱 Generating deterministic keys from seed")
...
def generate_deterministic_keys(seed: str) -> Tuple[bytes, bytes]:
    ...
    logger.debug(f"🌱 Generated deterministic keys (public key hash: {hashlib.sha256(public_key).hexdigest()[:8]})")
...
def generate_ephemeral_keys() -> Tuple[bytes, bytes]:
    ...
    logger.debug(f"✨ Generated ephemeral keys (public key hash: {hashlib.sha256(public_key).hexdigest()[:8]})")
...
def load_keys_from_path(key_path: Path) -> Tuple[bytes, bytes]:
    ...
    logger.debug(f"📁 Loaded keys from {key_path} (public key hash: {hashlib.sha256(public_key).hexdigest()[:8]})")
