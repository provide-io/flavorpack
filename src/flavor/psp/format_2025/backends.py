...
from flavor.psp.format_2025.constants import (
    ACCESS_AUTO, ACCESS_FILE, ACCESS_MMAP, ACCESS_STREAM,
...
class MMapBackend(Backend):
    ...
    def open(self, path: Path) -> None:
        ...
        logger.debug("🗺️ Opening mmap backend", path=str(path), size_bytes=file_size, size_mb=file_size/1024/1024)
        ...
        logger.debug("✅ MMap opened", elapsed_ms=elapsed*1000, pages=file_size//PAGE_SIZE)
    
    def close(self) -> None:
        """Close memory map and file."""
        logger.debug("🔒 Closing mmap backend", path=str(self.path) if self.path else None, tracked_views=len(self._views))
    ...
    def read_at(self, offset: int, size: int) -> memoryview:
        ...
        if offset + size > len(self.mmap):
            logger.error("❌ Read beyond bounds", offset=offset, size=size, file_size=len(self.mmap))
            raise ValueError(f"Read beyond file bounds: offset={offset}, size={size}, file_size={len(self.mmap)}")
        ...
        elapsed = time.perf_counter() - start_time
        logger.debug("🔍 MMap read_at", offset=offset, size=size, elapsed_us=elapsed*1000000, zero_copy=True)
    ...
    def prefetch(self, offset: int, size: int) -> None:
        ...
        logger.debug("📥 Prefetching pages", offset=offset, size=size, pages=size//PAGE_SIZE)
...
class FileBackend(Backend):
    ...
    def open(self, path: Path) -> None:
        ...
        self.path = path
        file_size = path.stat().st_size
        logger.debug("📁 Opening file backend", path=str(path), size_bytes=file_size, buffer_size=64*1024)
        
        # Use buffered I/O for better performance
        self.file = open(path, 'rb', buffering=64 * 1024)
    ...
    def close(self) -> None:
        """Close the file."""
        logger.debug("🔒 Closing file backend", path=str(self.path) if self.path else None, cache_entries=len(self._cache))
    ...
    def read_at(self, offset: int, size: int) -> bytes:
        ...
        elapsed = time.perf_counter() - start_time
        logger.debug("📂 File read_at", offset=offset, size=size, elapsed_us=elapsed*1000000, cached=size<=4096)
...
class StreamBackend(Backend):
    ...
    def open(self, path: Path) -> None:
        ...
        self.path = path
        self.file = open(path, 'rb', buffering=self.chunk_size)
    ...
    def stream_slot(self, descriptor: SlotDescriptor, chunk_size: Optional[int] = None):
        """Stream slot data in chunks."""
...
class HybridBackend(Backend):
    ...
    def __init__(self, header_size: int = 1024 * 1024):  # 1MB default
        self.header_size = header_size
        self.file: Optional[BinaryIO] = None
        self.header_mmap: Optional[mmap.mmap] = None
        self.path: Optional[Path] = None
...
def create_backend(mode: int = ACCESS_AUTO, path: Optional[Path] = None) -> Backend:
    ...
    if mode == ACCESS_AUTO:
        ...
        if path and path.exists():
            ...
            if file_size > 1024 * 1024:
                mode = ACCESS_MMAP
                logger.debug("🤖 Auto-selected mmap backend", file_size_mb=file_size/1024/1024)
            # Use streaming for very large files on limited memory
            elif file_size > 100 * 1024 * 1024 and sys.platform == "win32":
                mode = ACCESS_STREAM
                logger.debug("🤖 Auto-selected stream backend", file_size_mb=file_size/1024/1024, platform=sys.platform)
            else:
                mode = ACCESS_FILE
