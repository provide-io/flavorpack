...
class SafeOptimizer:
    ...
    def remove_cache_files(self, wheel_path: Path) -> int:
        ...
        with zipfile.ZipFile(wheel_path, 'r') as zin:
            with zipfile.ZipFile(wheel_path.with_suffix('.tmp'), 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as zout:
                for item in zin.infolist():
                    if '__pycache__' not in item.filename and not item.filename.endswith('.pyc'):
    ...
    def remove_docs(self, wheel_path: Path) -> int:
        ...
        with zipfile.ZipFile(wheel_path, 'r') as zin:
            with zipfile.ZipFile(wheel_path.with_suffix('.tmp'), 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as zout:
                for item in zin.infolist():
                    skip = any(item.filename.endswith(ext) for ext in ['.md', '.rst', '.txt'])
                    skip = skip or any(d in item.filename for d in ['docs/', 'doc/', 'examples/'])
    ...
    def strip_type_hints(self, wheel_path: Path) -> int:
        ...
        with zipfile.ZipFile(wheel_path, 'r') as zin:
            with zipfile.ZipFile(wheel_path.with_suffix('.tmp'), 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as zout:
                for item in zin.infolist():
                    if not item.filename.endswith('.pyi') and 'py.typed' not in item.filename:
                        zout.writestr(item, zin.read(item.filename))
                    else:
                        saved += item.file_size
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.Import):
                        for alias in node.names:
                            imports.setdefault(alias.name.split('.')[0], set()).add(str(py_file))
                    elif isinstance(node, ast.ImportFrom):
                        if node.module:
                            imports.setdefault(node.module.split('.')[0], set()).add(str(py_file))
            except:
                pass  # Skip unparseable files
        
        return imports
    
    def suggest_removals(self, required_packages: Set[str], installed_packages: Set[str]) -> Set[str]:
        ...
        always_keep = {
            'pip', 'setuptools', 'wheel', 'certifi', 'urllib3',
            'charset-normalizer', 'idna', 'six', 'typing-extensions'
        }
...
class ExplicitWhitelist:
    ...
    def filter_wheels(self, wheels_dir: Path, output_dir: Path):
