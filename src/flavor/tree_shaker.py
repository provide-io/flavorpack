...
class PythonTreeShaker:
    ...
    def optimize_wheel(self, wheel_path: Path, output_path: Path) -> dict[str, int]:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for py_file in py_files:
                ...
                if module_name in self.used_symbols:
                    ...
                    module_name.startswith(used + ".") for used in self.used_symbols
                ):
        ...
        stats["reduction_percent"] = int(
            (stats["size_reduction"] / stats["original_size"]) * 100
        )
...
def tree_shake_dependencies(
    entry_point: str, project_root: Path, wheels_dir: Path, output_dir: Path
    ...
    for wheel_path in wheels_dir.glob("*.whl"):
        ...
        if stats["size_reduction"] > 0:
            print(
                f"  Reduced by {stats['size_reduction'] / 1024:.1f} KB "
