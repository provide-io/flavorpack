...
class RetryConfig:
    ...
    jitter: bool = True
    retryable_exceptions: tuple[type[Exception], ...] = (IOError, OSError, ConnectionError)
    ...
    def _on_failure(self) -> None:
        ...
        with self._lock:
            ...
            if self.state == CircuitState.HALF_OPEN:
                ...
                self.state = CircuitState.OPEN
                logger.warning(
                    f"Circuit breaker entering OPEN state ({self.failures} failures)"
                )
...
class CheckpointManager:
    ...
    def __init__(self, checkpoint_dir: Path | None = None) -> None:
        self.checkpoint_dir = (
            checkpoint_dir or Path(tempfile.gettempdir()) / "flavor" / "checkpoints"
        )
    ...
    def save_checkpoint(self, operation_id: str, state: dict) -> None:
        ...
        with checkpoint_file.open("w") as f:
            json.dump({"state": state, "timestamp": time.time(), "pid": os.getpid()}, f)
