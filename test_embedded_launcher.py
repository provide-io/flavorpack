...
def test_packager_with_embedded_launcher(packager_name, launcher_name):
...
""")
    ...
    # Get packager path
    packager_path = Path(f"./flavor-{packager_name}") if packager_name != "python" else "flavor"
    ...
    try:
        if packager_name == "python":
            ...
            # Go/Rust packagers
            cmd = [str(packager_path), "package",
                   "--manifest", str(manifest_path),
                   "--output", output_file,
                   "--launcher", str(launcher_path)]
        
        output = run_command(cmd, cwd=test_dir)
        ...
        result = subprocess.run([str(package_path), "--version"], 
