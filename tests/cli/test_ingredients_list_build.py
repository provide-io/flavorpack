...
class TestIngredientList:
    """Test suite for 'flavor ingredients list' command."""

    @patch("flavor.ingredients.manager.IngredientManager")
    def test_list_empty(self, mock_manager_class: Mock) -> None:
        """Test list command with no ingredients."""
        mock_manager = Mock()
        mock_manager.list_ingredients.return_value = {"launchers": [], "builders": []}
        ...
        assert "No ingredients found" in result.output
        assert "flavor ingredients build" in result.output
    ...
    def test_list_with_launchers_only(self, mock_run: Mock, mock_manager_class: Mock, tmp_path: Path) -> None:
        ...
        assert result.exit_code == 0
        assert "Available Flavor Ingredients" in result.output
    ...
    def test_list_multiple_launchers_sorted(
        ...
        """Test list command sorts ingredients by name."""
        mock_manager = Mock()
        launcher_z = IngredientInfo(
            name="z-launcher", path=tmp_path / "z", type="launcher", language="go", size=1_000_000
        )
        launcher_a = IngredientInfo(
...
class TestIngredientBuild:
    ...
    def test_build_all_success(self, mock_manager_class: Mock, tmp_path: Path) -> None:
        ...
        mock_manager.build_ingredients.return_value = built_paths
        mock_manager_class.return_value = mock_manager

        runner = CliRunner()
        result = runner.invoke(cli_main, ["ingredients", "build"])

        assert result.exit_code == 0
        assert "Building all ingredients" in result.output
        assert "Built 2 ingredient(s)" in result.output
        assert "launcher.bin" in result.output
        assert "builder.bin" in result.output
        mock_manager.build_ingredients.assert_called_once_with(language=None, force=False)
    ...
    def test_build_go_only(self, mock_manager_class: Mock, tmp_path: Path) -> None:
        ...
        mock_manager.build_ingredients.return_value = [built_path]
        mock_manager_class.return_value = mock_manager

        runner = CliRunner()
        result = runner.invoke(cli_main, ["ingredients", "build", "--lang", "go"])

        assert result.exit_code == 0
        assert "Building go ingredients" in result.output
        assert "Built 1 ingredient(s)" in result.output
        mock_manager.build_ingredients.assert_called_once_with(language="go", force=False)
    ...
    def test_build_rust_only(self, mock_manager_class: Mock, tmp_path: Path) -> None:
        ...
        runner = CliRunner()
        result = runner.invoke(cli_main, ["ingredients", "build", "--lang", "rust"])

        assert result.exit_code == 0
        assert "Building rust ingredients" in result.output
        mock_manager.build_ingredients.assert_called_once_with(language="rust", force=False)
    ...
    def test_build_with_force_flag(self, mock_manager_class: Mock, tmp_path: Path) -> None:
        ...
        runner = CliRunner()
        result = runner.invoke(cli_main, ["ingredients", "build", "--force"])

        assert result.exit_code == 0
        mock_manager.build_ingredients.assert_called_once_with(language=None, force=True)

    @patch("flavor.ingredients.manager.IngredientManager")
    def test_build_no_ingredients_built(self, mock_manager_class: Mock) -> None:
        """Test build command when no ingredients are built."""
        mock_manager = Mock()
        mock_manager.build_ingredients.return_value = []
        mock_manager_class.return_value = mock_manager
        ...
        assert result.exit_code == 0
        assert "No ingredients were built" in result.output
