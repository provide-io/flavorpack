...
class TestSlotTableReading:
    ...
    def test_read_slot_table_structure(self, test_bundle_with_slots):
        ...
        assert slot0['lifecycle'] in [0, 1, 2, 3]  # persistent, volatile, temporary, install
    ...
    def test_slot_table_binary_format(self, test_bundle_with_slots):
        """Test that slot table entries are exactly 24 bytes each."""
        launcher = PSPFLauncher(test_bundle_with_slots)
        index = launcher.read_index()
        
        # Slot table size should be multiple of 24 bytes
        assert index.slot_table_size % 24 == 0
        
        # Number of entries should match
        expected_entries = index.slot_count
        actual_size = index.slot_table_size
        assert actual_size == expected_entries * 24
...
class TestSlotExtraction:
    ...
    def bundle_with_compressed_slots(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            slot1_path = tmpdir / "large.txt"
            slot1_path.write_text(large_content)
            ...
            slots = [
                SlotMetadata(
                    ...
                    compressed_size=0,  # Will be set by builder
                    checksum=hashlib.sha256(large_content.encode()).hexdigest(),
                    encoding="gzip",
                    purpose="payload",
                    lifecycle="persistent",
                    path=slot1_path
            ...
            bundle_path = tmpdir / "compressed.pspf"
            builder = PSPFBuilder()
...
""")
            ...
            bundle_path = tmpdir / "executable.pspf"
            builder = PSPFBuilder()
            ...
            metadata = {
                ...
                "package": {
                    "name": "hello-pspf",
                    "version": "1.0.0",
                    "entry_point": "python {slot:0}"
                },
                "execution": {
                    "command": "python {slot:0}",
                    "primary_slot": 0
                }
            }
    ...
    def test_execute_bundle(self, executable_bundle):
        ...
        assert "hello-pspf" in result['stdout']
