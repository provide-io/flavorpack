...
def test_full_cross_language_matrix():
...
""")
        ...
        for packager_name, packager_func in packagers.items():
            ...
            for launcher_name, launcher_path in launchers.items():
                ...
                # Create executable
                executable = temp_dir / f"test-{packager_name.lower()}-{launcher_name.lower()}"
                ...
                if result.returncode == 0:
                    ...
                    if launcher_name == "Rust" and "🦀🚀" in result.stderr:
                        results.append(f"   🦀 Rust emojis confirmed!")
                else:
                    results.append(f"❌ {packager_name} + {launcher_name}: {result.stderr}")
