...
def test_python_packager_go_launcher_compatibility() -> None:
...
def test_keygen_compatibility() -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        result = subprocess.run(
            [
                ...
                "keygen",
                "--private-key",
                str(go_keys / "provider-private.key"),
                "--public-key",
                str(go_keys / "provider-public.key"),
            ],
            capture_output=True,
            text=True,
        )
