...
class TestPackagerCompatibility:
    ...
    def test_all_packagers_produce_same_package(
        self, test_provider_dir, packager_binaries
    ) -> None:
        ...
        for name, cmd in packager_binaries.items():
            with tempfile.TemporaryDirectory() as output_dir:
                ...
                if name == "python":
                    ...
                    full_cmd = [*cmd, "build", "--out", str(output_dir / "test.flavor"), "--payload-dir", str(test_provider_dir), "--package-key", str(test_provider_dir / "keys" / "provider-private.key"), "--public-key", str(test_provider_dir / "keys" / "provider-public.key"), "--launcher-bin", str(ensure_go_binary("flavor-launcher-go"))]

                result = subprocess.run(
                    full_cmd, cwd=test_provider_dir, capture_output=True, text=True
                )

                assert result.returncode == 0, (
                    f"{name} packager failed: {result.stderr}"
                )
        ...
        checksums = {
            name: pkg["metadata"]["checksum"] for name, pkg in packages.items()
        }
        ...
        assert len(unique_checksums) == 1, (
            f"Packagers produced different packages: {checksums}"
        )
    ...
    def test_packager_verify_commands(self, test_provider_dir, packager_binaries) -> None:
        """Test that all packagers can verify packages from any packager."""
        if len(packager_binaries) < 2:
            pytest.skip(
                "Need at least 2 packager implementations to test cross-verification"
            )
        ...
        for name, cmd in packager_binaries.items():
            with tempfile.TemporaryDirectory() as output_dir:
                ...
                if name == "python":
                    full_cmd = [*cmd, "package", "--manifest", str(test_provider_dir / "pyproject.toml")]
                    subprocess.run(
                        full_cmd, cwd=test_provider_dir, check=True, capture_output=True
                    )
                    package_path = test_provider_dir / "dist" / "test.flavor"
                    ...
                    full_cmd = [*cmd, "build", "--out", str(output_path), "--payload-dir", str(test_provider_dir), "--package-key", str(test_provider_dir / "keys" / "provider-private.key"), "--public-key", str(test_provider_dir / "keys" / "provider-public.key"), "--launcher-bin", str(ensure_go_binary("flavor-launcher-go"))]
                    subprocess.run(
                        full_cmd, cwd=test_provider_dir, check=True, capture_output=True
                    )
...
class TestPackagerConsistency:
    ...
    def test_packager_deterministic(
        self, test_provider_dir, packager_binaries, packager_name
    ) -> None:
        ...
        for _i in range(2):
            with tempfile.TemporaryDirectory() as output_dir:
                ...
                if packager_name == "python":
                    ...
                    package_path = output_dir / "test.flavor"
                    full_cmd = [*cmd, "build", "--out", str(package_path), "--payload-dir", str(test_provider_dir), "--package-key", str(test_provider_dir / "keys" / "provider-private.key"), "--public-key", str(test_provider_dir / "keys" / "provider-public.key"), "--launcher-bin", str(ensure_go_binary("flavor-launcher-go"))]
                    subprocess.run(
                        full_cmd, cwd=test_provider_dir, check=True, capture_output=True
                    )
