...
from flavor.api import build_package_from_manifest, generate_keys
...
""")
    ...
    subprocess.run(
        ["tar", "-czf", str(payload_tgz), "-C", str(payload_dir), "."], check=True
    )
...
""")
    ...
    result = subprocess.run(
        [
            ...
            str(payload_dir),
            "--package-key",
            str(keys_dir / "provider-private.key"),
            "--public-key",
            str(keys_dir / "provider-public.key"),
            "--out",
            str(output_path),
        ],
        capture_output=True,
...
def test_launcher_compatibility(
    ...
    output_name = (
        f"pspf-test-{packager_abbrev}-builder-{launcher_abbrev}-launcher.flavor"
    )
    ...
    result = subprocess.run(
        [str(output_path)], capture_output=True, text=True, env=env, timeout=30
    )
...
""")
        ...
        try:
            rust_launcher_dir = Path(
                "/REDACTED_ABS_PATH"
            )
            rust_launcher = (
                rust_launcher_dir / "target" / "release" / "flavor-launcher-rs"
            )
        ...
        for packager_name, package_path in packages.items():
            for launcher_name, launcher_path in launchers.items():
                if packager_name == "Python" and launcher_name == "Python":
                    ...
                    packager_abbrev = {"Python": "py", "Go": "go", "Rust": "rs"}[
                        packager_name
                    ]
                ...
                print(
                    f"{packager_name:<10} {launcher_name:<10} {'✅' if success else '❌':<10} {notes}"
                )
        ...
        for output_file in sorted(output_files):
            if output_file.exists():
                ...
                print(
                    f"{output_file.name:<50} {size:>10,} bytes  SHA256: {checksum[:32]}..."
                )
        ...
        if len(checksum_groups) == 1:
            ...
            print(
                "   This proves all packagers produce functionally identical packages!"
            )
        else:
