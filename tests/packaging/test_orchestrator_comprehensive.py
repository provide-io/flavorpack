...
"""
    ...
    def test_build_package_creates_correct_structure(
        self, mock_tempdir, mock_ensure_go, mock_run, orchestrator
    ) -> None:
    ...
    @patch("flavor.packaging.orchestrator.subprocess.run")
    def test_python_environment_creation(self, mock_run, orchestrator) -> None:
        """Test Python virtual environment is created correctly."""
        mock_run.return_value = Mock(returncode=0, stdout="", stderr="")

        with tempfile.TemporaryDirectory() as temp_dir, patch(
            "flavor.packaging.orchestrator.tempfile.TemporaryDirectory"
        ) as mock_tempdir:
            mock_tempdir.return_value.__enter__ = Mock(return_value=temp_dir)
            mock_tempdir.return_value.__exit__ = Mock(return_value=None)

            with patch(
                "flavor.packaging.orchestrator.ensure_go_binary"
            ) as mock_ensure_go:
                mock_ensure_go.return_value = Path("/fake/flavor-packager")
                    ...
                    if len(args) >= 4 and args[0] == "uv" and args[1] == "venv":
                        if str(Path(temp_dir) / "payload") in args[2]:
                            if "--python" in args and "python3.13" in args:
                                ...
                                break
                assert venv_called, (
                    "uv venv should have been called with correct arguments"
                )

    @patch("flavor.packaging.orchestrator.subprocess.run")
    def test_dependencies_installation(self, mock_run, orchestrator, tmp_path) -> None:
                    ...
                    for call in calls:
                        ...
                        if isinstance(args, list) and len(args) >= 4:
                            if (
                                ...
                                if any(
                                    str(tmp_path).split("/private")[-1] in str(arg)
                                    for arg in args
                                ):
                                    main_install = True
                                    break
    ...
    @patch("flavor.packaging.orchestrator.subprocess.run")
    def test_uv_binary_copy(self, mock_run, orchestrator) -> None:
                ...
                with patch("pathlib.Path.exists") as mock_exists:
                    ...
                    with patch("shutil.copy2") as mock_copy:
                        ...
                        mock_copy.assert_called_once_with(
                            "/opt/homebrew/bin/uv", Path(temp_dir) / "uv"
                        )

    @patch("flavor.packaging.orchestrator.subprocess.run")
    def test_packager_command_construction(self, mock_run, orchestrator, tmp_path) -> None:
    ...
    @patch("flavor.packaging.orchestrator.subprocess.run")
    def test_tarball_creation_and_content(self, mock_run, orchestrator) -> None:
