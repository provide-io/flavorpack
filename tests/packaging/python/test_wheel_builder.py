...
class TestWheelBuilder:
    ...
    def test_build_wheel_with_options(self, mock_run_command):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = self.wheel_builder.build_wheel_from_source(
                python_exe, source_path, wheel_dir,
                use_isolation=False, build_options=build_options
            )
    ...
    def test_resolve_dependencies_fallback_to_pip_tools(self, mock_run_command):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(self.wheel_builder.uv, 'compile_requirements') as mock_uv_compile:
                ...
                result = self.wheel_builder.resolve_dependencies(
                    python_exe, requirements_file=requirements_file, output_dir=output_dir
                )
    ...
    def test_build_and_resolve_project_complete(self, mock_run_command):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(self.wheel_builder, 'resolve_dependencies') as mock_resolve:
                with patch.object(self.wheel_builder, 'download_wheels_for_resolved_deps') as mock_download:
                    # Setup mock returns
                    ...
                    result = self.wheel_builder.build_and_resolve_project(
                        python_exe, project_dir, build_dir,
    ...
    def test_dependency_resolution_has_uv_fallback(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch.object(self.wheel_builder.uv, 'compile_requirements') as mock_uv:
                ...
                result = self.wheel_builder.resolve_dependencies(
                    python_exe, packages=packages, output_dir=output_dir, use_uv_for_resolution=True
                )
