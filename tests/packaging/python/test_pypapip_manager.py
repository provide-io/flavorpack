...
class TestPyPaPipManager:
    ...
    def test_get_pypapip_wheel_cmd_basic(self):
        ...
        expected = [
            ...
            "--wheel-dir", "/tmp/wheels", "/tmp/mypackage"
        ]
        assert cmd == expected
    ...
    def test_get_pypapip_wheel_cmd_no_deps(self):
        ...
        expected = [
            "/usr/bin/python3", "-m", "pip", "wheel", 
            "--wheel-dir", "/tmp/wheels", "--no-deps", "/tmp/mypackage"
        ]
        assert cmd == expected
...
class TestPyPaPipManagerCriticalFeatures:
    ...
    def test_linux_platforms_always_get_manylinux_tags(self, mock_arch, mock_os):
        ...
        for arch, expected_tag in [
            ...
            cmd = self.pip_manager._get_pypapip_download_cmd(
                Path("/usr/bin/python3"), Path("/tmp"), 
                packages=["test"], binary_only=True
            )
    ...
    def test_binary_only_flag_always_present_for_downloads(self):
        ...
        cmd2 = self.pip_manager._get_pypapip_download_cmd(
            python_exe, dest_dir, requirements_file=Path("/tmp/req.txt"), binary_only=True
        )
