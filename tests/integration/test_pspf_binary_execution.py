...
"""
    ...
    def test_build_minimal_provider(self, test_provider_dir) -> None:
        ...
        assert size_mb > 30, (
            f"Package too small ({size_mb}MB), Python probably not embedded"
        )

    def test_verify_built_package(self, test_provider_dir) -> None:
    ...
    def test_extract_and_inspect_package_contents(self, test_provider_dir) -> None:
        ...
        with open(artifact_path, "rb") as f:
            ...
            assert (
                ...
                or len(header) == 4
            )  # Any 4-byte header is acceptable for now

    @pytest.mark.skipif(
        os.environ.get("CI") == "true", reason="Binary execution tests may fail in CI"
    )
    def test_execute_built_binary(self, test_provider_dir) -> None:
        ...
        result = subprocess.run(
            [str(artifact_path), "--help"], capture_output=True, text=True, timeout=10
        )
    ...
    def test_execute_with_terraform_protocol(self, test_provider_dir) -> None:
        ...
        env = os.environ.copy()
        env["TF_PLUGIN_MAGIC_COOKIE"] = (
            "d602bf8f470bc67ca7faa0386276bbdd4330efaf76d1a219cb4d6991ca9872b2"
        )
        env["PLUGIN_PROTOCOL_VERSIONS"] = "6"

        result = subprocess.run(
            [str(artifact_path)], capture_output=True, text=True, env=env, timeout=10
        )
    ...
    def test_package_caching_behavior(self, test_provider_dir) -> None:
    ...
    def test_build_with_dependencies(self, test_provider_dir, tmp_path) -> None:
...
""")
        ...
        content = content.replace(
            "dependencies = []", f'dependencies = ["test-dep @ file://{dep_dir}"]'
        )
    ...
    def test_payload_archive_structure(self, test_provider_dir, tmp_path) -> None:
        ...
        orchestrator = PackagingOrchestrator(
            package_integrity_key_path=str(
                test_provider_dir / "keys" / "provider-private.key"
            ),
