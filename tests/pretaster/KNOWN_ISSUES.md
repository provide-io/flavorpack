...
**Decision:** The Rust launcher (`flavor-rs-launcher`) is not supported on Windows
(AMD64 or ARM64). All Windows packages must use Go builder + Go launcher.

**Symptoms on Windows:**
- Signal 9 (SIGKILL) / abnormal termination when the Rust launcher binary is invoked
- Root cause: unknown; possibly a Rust runtime/allocator incompatibility with the
  Windows PE loader or an issue with the PSPF reading path on Windows

**Rust builder status:** ✅ Works on Windows (streaming I/O fix applied)
...
it is fixable. The code is preserved; just disable its use in CI until fixed.
...
When the Rust launcher is fixed for Windows, update `build-pretaster.sh` and the
pipeline YAML to remove the Windows-specific launcher selection.