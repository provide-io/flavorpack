...
class CrossLangTester:
    ...
    def __init__(self, verbose=False, json_output=False) -> None:
        ...
        if ingredients_dir:
            ...
            # Look up the directory tree for an ingredients/bin directory
            current = Path.cwd()
            for parent in [current, *current.parents]:
                ingredients_bin = parent / "ingredients" / "bin"
                logger.trace("Checking for ingredients", path=str(ingredients_bin))
        ...
        if (current / "pyproject.toml").exists() and "taster" in str(current):
            ...
            for parent in [current, *list(current.parents)]:
                taster_path = parent / "helpers/taster"
