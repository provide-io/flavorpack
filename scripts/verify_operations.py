...
from flavor.psp.format_2025.operations import (
    ...
    OP_NONE, OP_TAR, OP_GZIP, OP_BZIP2, OP_ZSTD, OP_AES256_GCM
)
...
def verify_python_operations():
    ...
    for ops, expected_packed, description in test_cases:
        ...
        if packed != expected_packed:
            print(f"❌ Python pack failed for {description}: got 0x{packed:016x}, want 0x{expected_packed:016x}")
            all_passed = False
        else:
...
def compare_test_vectors():
    ...
    test_file = Path("ingredients/flavor-go/pkg/psp/format_2025/testdata/operations.json")
