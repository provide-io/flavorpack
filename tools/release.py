...
def build_wheels(platforms: Optional[List[str]] = None) -> List[Path]:
...
def validate_wheels(wheels: List[Path]) -> bool:
    ...
    for wheel in wheels:
        result = run_command([
            sys.executable, "tools/validate_wheel.py",
            str(wheel)
        ], cwd=get_project_root())
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Orchestrate Flavor release process"
    )
    parser.add_argument(
        "--version",
    ...
    parser.add_argument(
        "--skip-ingredients",
        action="store_true",
        help="Skip building ingredients"
    )
    parser.add_argument(
        "--skip-validation",
        action="store_true",
    ...
    parser.add_argument(
        "--push-tag",
        action="store_true",
        help="Push git tag to origin"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Perform dry run (no uploads or tags)"
    )
...
""")
    ...
    if not args.dry_run:
        ...
        if branch not in ["main", "master", "develop"]:
            response = input(f"⚠️  Not on main branch. Continue? (y/N): ")
            if response.lower() != 'y':
                print("Aborted")
                return 1
