                ...
                if not ingredient_path.is_file():
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            messages.append(f"    ⚠️  Failed to run --version")
...
def validate_installation(wheel_path: Path) -> Tuple[bool, List[str]]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = subprocess.run(
            [str(pip), "install", str(wheel_path)],  # pip3 is critical for proper wheel installation
            capture_output=True,
            text=True
        )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help="Path to wheel file to validate"
    )
    parser.add_argument(
        "--all",
        action="store_true",
        help="Validate all wheels in dist/"
    )
    parser.add_argument(
        "--full",
        action="store_true",
