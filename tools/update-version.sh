...
sed -i '' "s/^const version = \".*\"/const version = \"$NEW_VERSION\"/" src/flavor-go/cmd/flavor-go-launcher/main.go
sed -i '' "s/^const version = \".*\"/const version = \"$NEW_VERSION\"/" src/flavor-go/cmd/flavor-go-builder/main.go

# Update Rust components
sed -i '' "s/version = \".*\"/version = \"$NEW_VERSION\"/" src/flavor-rs/Cargo.toml
# Use a more precise pattern for Rust const to avoid multiple replacements
sed -i '' "s/^const VERSION: &str = \".*\";/const VERSION: \&str = \"$NEW_VERSION\";/" src/flavor-rs/src/bin/flavor-rs-builder.rs
sed -i '' "s/^const VERSION: &str = \".*\";/const VERSION: \&str = \"$NEW_VERSION\";/" src/flavor-rs/src/bin/flavor-rs-launcher.rs
