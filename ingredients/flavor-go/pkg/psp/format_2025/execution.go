...
func runBundleWithCwd(exePath string, args []string, userCwd string, logger hclog.Logger) (*exec.Cmd, error) {
	...
	if !workenvValid {
		...
		packageMetadataDir := filepath.Join(tempExtractDir, fmt.Sprintf(".%s.pspf", paths.Name()), "package")
		if err := os.MkdirAll(packageMetadataDir, 0755); err != nil {
		...
		logger.Debug("📝 Wrote metadata to temp location", "path", metadataFile)
